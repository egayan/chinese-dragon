...
if (isset($_GET['freeze_out'])) {
    ...
    $freeze->execute([$_GET['freeze_out']]);

    $sql = $pdo->prepare('select * from client where client_id = ?');
    $sql->execute([$_GET['freeze_out']]);
    ...
    foreach ($sql as $row) {
        ...
        echo '</form></div>';
        echo '</td></tr>';
        echo '<tr><td><div align="center"><button><a href="account_management.php?freeze_id=', $row['client_id'], '"><h2>凍結</h2></a></div></button></p></td></tr>';
    ...
    if (isset($_GET['client_id'])) {
        ...
        $f_check->execute([$_GET['client_id']]);
        foreach ($f_check as $f_c) {
            $fcid = $f_c['freeze'];
        }
    } else if (isset($_GET['freeze_id'])) {
        $f_check = $pdo->prepare('select * from client where client_id = ?');
        $f_check->execute([$_GET['freeze_id']]);
        foreach ($f_check as $f_c) {
            $fcid = $f_c['freeze'];
        }
    } else if (isset($_POST['id'])) {
        $f_check = $pdo->prepare('select * from client where client_id = ?');
        $f_check->execute([$_POST['id']]);
        foreach ($f_check as $f_c) {
            $fcid = $f_c['freeze'];
        }
    }
    ...
    if ($fcid == 2) {
        if ($_SERVER["REQUEST_METHOD"] == 'POST') {
            ...
            foreach ($sql as $row) {
                ...
                echo '</form></div>';
                echo '</td></tr>';
                echo '<tr><td><div align="center"><button><a href="account_management.php?freeze_out=', $row['client_id'], '"><h2>凍結解除</h2></a></div></button></p></td></tr>';
                echo '<tr><td><div align="center"><button><a href="account.php">戻る</a></button></div></td></tr>';
            }
            echo '</table>';
        } else {
            ...
            if (isset($_GET['client_id'])) {
                $sql = $pdo->prepare('select * from client where client_id = ?');
                $sql->execute([$_GET['client_id']]);
            } else if (isset($_GET['freeze_id'])) {
                $sql = $pdo->prepare('select * from client where client_id = ?');
                $sql->execute([$_GET['freeze_id']]);
            } else if (isset($_POST['id'])) {
                $sql = $pdo->prepare('select * from client where client_id = ?');
                $sql->execute([$_POST['id']]);
            }
